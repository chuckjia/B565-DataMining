function F = apriori( dset, candGenRule, minsup, minconf, itemNames )
...
fprintf("  - Min Support = %1.6f, Min Confidence = %1.6f\n\n", minsup, minconf);
...
for lev = 2:numLev
    ...
    for freqSetNo = 1:numFreqSet
        ...
        for setNo = 1:length(H1)
            itemset = setdiff(F{lev}{freqSetNo}, H1{setNo});
            suppCount = suppCountCache{lev}{freqSetNo};
            conf = suppCount / calcSuppCount(dset, itemset);
            if (conf >= minconf)
                printRule(itemset, H1{setNo}, suppCount / length(dset), conf, itemNames);
            end
        end
        ap_genrules(F, suppCountCache, lev, freqSetNo, H1, dset, minconf, itemNames);
    end
end
